...
import { StableMarket } from "@/components/sections";
import { ConnectPanel } from "@/components/wallet-connect";
import { Button, buttonVariants } from "@/components/ui/button";
import { SITE } from "@/lib/constants";
import { getNeurMarket } from "@/lib/market";
import { useI18n } from "@/lib/i18n";
import { cn, copyText } from "@/lib/utils";
import { watchSplToken } from "@/lib/watch-token";

export const Route = createFileRoute("/neur")({
  loader: async () => {
    let quote = null;
    try {
      quote = await getNeurMarket();
    } catch {
      quote = null;
    }
    return { quote };
  ...
  component: NeurPage,
  head: () => ({
    meta: [{ title: "nEUR — Nexa FLY" }],
    links: [{ rel: "canonical", href: "https://nexafly.org/neur" }],
  }),
});
  ...
  async function onWatch() {
    try {
      await watchSplToken({
        ...
        symbol: "nEUR",
        decimals: 4,
        image: `${window.location.origin}/neur.png`,
      });
      setWatch("ok");
    } catch {
      setWatch("fail");
      ...
      <main className="mx-auto max-w-6xl px-5 pt-24 pb-24">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <article className="rounded-lg bg-surface p-5 shadow-[0_0_0_1px_rgba(244,236,223,0.08)]">
            ...
            <p className="mt-2 font-display text-xl font-semibold">EUR 1:1</p>
          </article>
        ...
        <StableMarket kind="neur" quote={quote} bare />
            ...
            {c.compare.map((item, i) => (
              <article
                ...
                className={cn(
                  ...
                  i === 2 && "shadow-[0_0_0_1px_rgba(255,128,0,0.28)]",
                )}
              >
          ...
          <div className="mt-8 overflow-hidden rounded-lg bg-surface shadow-[0_0_0_1px_rgba(244,236,223,0.08)]">
            <dl>
              ...
              <div className="grid gap-1 px-5 py-4 sm:grid-cols-[11rem_1fr] sm:items-baseline">
                ...
                <dd className="break-all font-mono text-xs text-amber">{SITE.neurMint}</dd>
              </div>
            </dl>
          </div>
        </section>
